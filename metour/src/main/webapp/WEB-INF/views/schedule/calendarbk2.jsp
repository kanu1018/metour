...
<table id="calendarTable">
  ...
  <tr>
  	<c:forEach begin="1" end="8">
  		<c:if test="${DOW gt count}">
  			<c:set var="count" value="${count+1}"/>
  			<td class="empty">&nbsp;</td>
  			${count}
  		</c:if>
  		<c:if test="${DOW le count}">
  		<%if(isDate( Integer.parseInt(request.getAttribute("Year").toString()), 
  				Integer.parseInt(request.getAttribute("Month").toString()), display)){
